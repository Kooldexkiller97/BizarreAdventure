...
pokermon.add_family({"riolu", "lucario"})
pokermon.add_family({"bagon", "shelgon", "salamence"})
pokermon.add_family({"gfarfetchd", "sirfetchd"})
pokermon.add_family({"starly", "staravia", "staraptor"})
pokermon.add_family({"axew", "fraxure", "haxorus"})

Bizarre_config = SMODS.current_mod.config
...
pokermon_config = pokermon_config or {}
      ...
      if curr_pokemon.list and #curr_pokemon.list > 0 then
        for i, item in ipairs(curr_pokemon.list) do
          if (pokermon_config.jokers_only and not item.joblacklist) or not pokermon_config.jokers_only  then
            ...
            pokermon.Pokemon(item, "Bizarre", nil)
          end
        end
      end
    end
